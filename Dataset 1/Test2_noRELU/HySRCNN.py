...
def build_img_pair(image, img_size):
    ...
    return img_norm[:,6:26,6:26,:], img_inter_norm
...
if __name__ == '__main__':
    ...
    with tf.name_scope('conv_1_feature_extraction'):
    	...
    	h_conv1 = conv2d(img_LR, W_conv1) + b_conv1
    	# summaries
    	tf.summary.histogram('histogram_W1', W_conv1)
    	tf.summary.histogram('histogram_b1', b_conv1)
    ...
    with tf.name_scope('conv_2_nonlinear_mapping'):
    	...
    	tf.summary.histogram('histogram_b2', b_conv2)
    ...
    with tf.name_scope('conv_3_reconstruction'):
    	...
    	tf.summary.histogram('histogram_b3', b_conv3)

    with tf.name_scope('evaluate'):
        MSE_loss = tf.reduce_mean(tf.square(img_HR-h_conv3))
        output_clip = tf.clip_by_value(h_conv3,0.,1.) * 255.
    ...
    with tf.Session() as sess:
        ...
        for i in range(test_num):
            ...
            tup = tup + tuple([temp])
        test_HR, test_LR = build_img_pair(np.stack(tup, axis = 0),img_size)

        for iter in range(maxiter):
            batch_HR, batch_LR = build_img_pair(sess.run(train_batch),img_size)
            if iter%10 == 0:
                summary_train = sess.run(summary, {img_LR:batch_LR, img_HR:batch_HR})
                train_writer.add_summary(summary_train, iter)
                train_writer.flush()
                summary_test = sess.run(summary, {img_LR:test_LR, img_HR:test_HR})
                test_writer.add_summary(summary_test, iter)
                test_writer.flush()
            if iter%100 == 0:
                train_loss  = MSE_loss.eval({img_LR:batch_LR, img_HR:batch_HR})
                test_loss   = MSE_loss.eval({img_LR:test_LR, img_HR:test_HR})
                test_MSE  = MSE.eval({img_LR:test_LR, img_HR:test_HR})
                test_PSNR = PSNR.eval({img_LR:test_LR, img_HR:test_HR})
                print("iter step %d trainning batch loss %f"%(iter, train_loss))
                print("iter step %d test loss %f"%(iter, test_loss))
                print("iter step %d test MSE %f"%(iter, test_MSE))
                print("iter step %d test PSNR %f \n"%(iter, test_PSNR))
            if iter%100 == 0:
                saver.save(sess, './HySRCNN_logs/model.ckpt', global_step=iter)
            train_step.run({img_LR:batch_LR, img_HR:batch_HR})
        coord.request_stop()
        coord.join(threads)
