...
def build_img_pair(image, img_size):
    ...
    MSE_pre = np.mean(np.power(img_cast-img_inter_cast, 2))
    PSNR_pre = 10.*np.log10(255.*255./MSE_pre)
    print('MSE  without optimization: %f' % MSE_pre)
    print('PSNR without optimization: %f' % PSNR_pre)
    return img_norm[:,6:506,6:506,:], img_inter_norm, img_inter_norm[:,6:506,6:506,:], \
...
if __name__ == '__main__':
    ...
    with tf.name_scope('input_data'):
    	...
    	img_LR_pad = tf.placeholder(tf.float32, [None, conv_height, conv_width, image_depth])
    ...
    with tf.name_scope('evaluate'):
        clip_SR_image  = tf.clip_by_value(h_conv3+img_LR_pad,0.,1.) * (max_HR_image-min_HR_image) \
            +  min_HR_image
    ...
    with tf.Session() as sess:
        saver.restore(sess, './HySRCNN_logs/model.ckpt-14999')
        #summary = tf.summary.merge_all()
        #final_test_writer = tf.summary.FileWriter('./HySRCNN_logs/final_model_test', sess.graph)

        coord = tf.train.Coordinator()
        threads = tf.train.start_queue_runners(sess=sess, coord=coord)
        test_HR, test_LR, test_LR_pad, Range = build_img_pair(sess.run(test_batch), img_size)
        coord.request_stop()
        coord.join(threads)

        I = sess.run(cast_SR_image, \
                {img_HR:test_HR, img_LR:test_LR, img_LR_pad:test_LR_pad, \
                min_HR_image:Range[0], max_HR_image:Range[1], min_SR_image:Range[2], max_SR_image:Range[3]})
